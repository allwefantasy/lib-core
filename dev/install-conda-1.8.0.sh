...
if [ ! -x "$(command -v conda)" ]; then
  ...
  if [ "${_MACHINE}" = "Mac" ]; then
    _MINICONDA3_DOWNLOAD_URL="https://repo.anaconda.com/miniconda/Miniconda3-py37_4.10.3-MacOSX-x86_64.sh"
    ...
    _MINICONDA3_DOWNLOAD_URL="https://repo.anaconda.com/miniconda/Miniconda3-py37_4.10.3-Linux-x86_64.sh"
    # miniconda requires bzip2
    if [ ! -x "$(command -v bzip2)" ]; then
      apt-get update
...
if conda env list | grep -q 'lib-core-py-3613'; then
  echo "Conda env of lib-core-py-3613 already exists, skipping..."
else
  echo "Start to create a conda environment with python version 3.6.13: lib-core-py-3613..." &&
      conda create --name  ray1.8.0 python=3.6.13 &&
      echo "The conda environment is created."  
fi

source deactivate
source activate ray1.8.0

(. "${_CONDA_HOME}"/bin/activate ray1.8.0 &&
  ...
  pip config set global.index-url http://mirrors.aliyun.com/pypi/simple/ &&
   pip install --upgrade pyarrow==4.0.1 "ray[default]==1.8.0" aiohttp==3.7.4 "pandas~=1.0.5" requests  \
               "matplotlib~=3.3.4" "uuid~=1.30" pyjava opencv-python pyecharts seaborn sklearn keras tensorflow &&
